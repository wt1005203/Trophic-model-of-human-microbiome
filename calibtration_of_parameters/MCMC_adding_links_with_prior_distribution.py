...
def pred_error_addingLinks(i_train, x, m2b_ori, b2m_ori, x_ori):
    ...
    pred_errorTotal = pred_error2 + hyper_reg * pred_error3 - (pred_error4 - 20) * float(sys.argv[1]) # with reward
...
df_added_tables.to_csv('added_links_prior_distribution_hyper'+sys.argv[1]+'_run' + sys.argv[2]+'.csv')
...
def error_evaulation(i_subset):
    ...
    return [np.mean(corr_list[:,1]), np.mean(log_list[:,1])]

train_error_corr, train_error_log10 = error_evaulation(i_train)
test_error_corr, test_error_log10 = error_evaulation(i_test)

print("The average correlation coefficient and log10 accuracy of in sample (training) are:")
print(train_error_corr, train_error_log10)
print("The average correlation coefficient and log10 accuracy of out sample (test) are:")
print(test_error_corr, test_error_log10)
df_train_test = np.array([[train_error_corr, train_error_log10],[test_error_corr, test_error_log10]])
df_train_test = pd.DataFrame(df_train_test)
df_train_test.columns = ['correlation', 'log10']
df_train_test.index = ['train', 'test']

df_train_test.to_csv('train_test_prior_distribution_hyper'+sys.argv[1]+'_run' + sys.argv[2]+'.csv')