...
hyperparameter_numLinks = sys.argv[1]
hyperparameter_rewards = sys.argv[2]
whichReplicate = sys.argv[3]
...
def pred_error_addingLinks(i_train, x, m2b_ori, b2m_ori, x_ori):
    ...
    order_dev_list = np.zeros((i_train.shape[0], MAX_ID_metabolites))
    order_dev_metagenome_list = np.zeros((i_train.shape[0], MAX_ID_microbes))
    ...
    for pa in range(i_train.shape[0]):
        ...
        b_real = np.zeros((MAX_ID_microbes,))
        b_real[thai_metagenome_ID] = thai_metagenome.iloc[:, i_train[pa]] / np.sum(thai_metagenome.iloc[:, i_train[pa]])
        ...
        metabolome_measured[thai_metabolome_ID] = thai_metabolome.iloc[:, i_train[pa]]
    ...
    #hyper_reg = 0.001
    hyper_reg = float(hyperparameter_numLinks)

    #pred_errorTotal = pred_error1 + pred_error2 + hyper_reg * pred_error3
    #pred_errorTotal = pred_error2 + hyper_reg * pred_error3 # without reward
    pred_errorTotal = pred_error2 + hyper_reg * pred_error3 - (pred_error4 - 20) * float(hyperparameter_rewards) # with reward
...
i_train = np.sort(np.random.choice(range(41), 30, replace=False))
i_test = np.setdiff1d(range(41), i_train)
...
#df_added_tables.to_csv('./added_links_prior_distribution_hyperNumLinks.csv')
df_added_tables.to_csv('added_links_prior_distribution_hyperNumLinks'+hyperparameter_numLinks+'_hyperRewards'+hyperparameter_rewards+'_run'+whichReplicate+'.csv')

######## compute the in-sample error and out-of-sample errors:
def error_evaulation(i_subset):
    f_byproduct = 0.9
    f = f_byproduct * np.ones((MAX_ID_microbes,1));
    f[i_selfish] = 0.0;  # The byproduct/leakage fraction f for microbes that don't generate byproducts is set as 0.
    ...
    corr_list = np.zeros((i_subset.shape[0], 2))
    diet_list = np.zeros((i_subset.shape[0], 20))
    log_list = np.zeros((i_subset.shape[0], 2))
    numMetabolites_list = np.zeros((i_subset.shape[0]))
    for pa in range(i_subset.shape[0]):
        #print(pa)
        ######## Select an ID of a individual and pull out its experimentally measured metagenome:
        b_real = np.zeros((MAX_ID_microbes,))
        b_real[thai_metagenome_ID] = thai_metagenome.iloc[:, i_subset[pa]] / np.sum(thai_metagenome.iloc[:, i_subset[pa]])
        ...
        met_levels, met_leftover_levels = calc_metabolome(res.x, m2m_layer, numLevels_max, i_intake)
        metabolome_measured = np.zeros((MAX_ID_metabolites,))
        metabolome_measured[thai_metabolome_ID] = thai_metabolome.iloc[:, i_subset[pa]]
        metabolome_pred = met_levels[:, -1] + met_leftover_levels.sum(1) #np.dot(m2m_total, x_full)
        i_common = np.where(metabolome_measured * metabolome_pred > 1e-5)[0]
        metabolome_pred_common = metabolome_pred[i_common] / np.sum(metabolome_pred[i_common])
        metabolome_measured_common = metabolome_measured[i_common] / np.sum(metabolome_measured[i_common])
    ...
    return [np.mean(corr_list[:,1]), np.mean(log_list[:,1]), np.mean(numMetabolites_list)]

train_error_corr, train_error_log10, train_numMetabolites = error_evaulation(i_train)
test_error_corr, test_error_log10, test_numMetabolites = error_evaulation(i_test)

print("The average correlation coefficient and log10 accuracy of in sample (training) are:")
print(train_error_corr, train_error_log10, train_numMetabolites)
print("The average correlation coefficient and log10 accuracy of out sample (test) are:")
print(test_error_corr, test_error_log10, test_numMetabolites)
df_train_test = np.array([[train_error_corr, train_error_log10, train_numMetabolites], [test_error_corr, test_error_log10, test_numMetabolites]])
df_train_test = pd.DataFrame(df_train_test)
df_train_test.columns = ['correlation', 'log10', 'numMetabolites']
df_train_test.index = ['train', 'test']

df_train_test.to_csv('./train_test_prior_distribution_hyperNumLinks'+hyperparameter_numLinks+'_hyperRewards'+hyperparameter_rewards+'_run'+whichReplicate+'.csv')