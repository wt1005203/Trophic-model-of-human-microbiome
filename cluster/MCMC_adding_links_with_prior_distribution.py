...
for i in range(10000):
    ...
    i_x = np.random.choice(np.where(x==0)[0], 1, p=prior_prob[x==0]/np.sum(prior_prob[x==0]))
    x[i_x] = 1
    #x[i_x] = 1 - x[i_x]
    ...
    prior_prob = np.concatenate([prior_prob.flatten(), prior_prob.flatten()]) + 0.1
    prior_prob = prior_prob / np.sum(prior_prob)
    if np.random.uniform(0,1,1)[0] <= np.min([1, np.exp(5000*(error_before - error_after))]):
