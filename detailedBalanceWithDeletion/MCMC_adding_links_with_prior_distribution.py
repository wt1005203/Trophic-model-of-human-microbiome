...
df_modified_tables.to_csv('added_links_prior_distribution_run' + sys.argv[1]+'.csv')
...
df_error = np.array([[np.mean(corr_list[:,1]), np.mean(log_list[:,1]), np.mean(numMetabolites)]])
df_error = pd.DataFrame(df_error)
df_error.columns = ['correlation', 'log10', 'numMetabolites']

df_error.to_csv('error_prior_distribution_run' + sys.argv[1]+'.csv')